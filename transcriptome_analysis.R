...
setwd("~/Mestrado_NOVA/Primeiro_ano/2_MCMO/project/colorectal/")
...
sampleTypes <- gsub("_rep[123]", "", colnames(logTPMs))                      #regular expression
...
plotCol <- rep(c("indianred", "turquoise"), each=3)
barplot(logTPMs["MIR1307",], col=plotCol, las=1, main="MIR1307", ylab="Expression levels (logTPMs)",  cex.names = 1) #Gene positively correlated with PC1
legend("topleft", fill=unique(plotCol), legend=c("Hypoxia","Normoxia"), bty="n") 

barplot(logTPMs["ANGPTL4",], col=plotCol, las=1, main="ANGPTL4", ylab="Expression levels (logTPMs)") #Gene negatively correlated with PC1
legend("topright", fill=unique(plotCol), legend=c("Hypoxia","Normoxia"), bty="n")
...
#HIF factors table
HIF_expression <- readCounts_gene$counts[grepl("HIF", rownames(readCounts_gene$counts)), ]
...
design_matrix
contrast_matrix <- makeContrasts(Hypoxia-Normoxia, levels = design_matrix)
contrast_matrix
...
#########################################################################################################################
# Explore transcriptome alterations at a pathway level (lesson6) ####
#########################################################################################################################

# Write tables upReg, downReg and DEGs as txts

write.table(upReg, col.names = F, quote = F, file = "upReg.txt", sep = "\t",
            row.names = FALSE)

write.table(downReg, col.names = F, quote = F, file = "downReg.txt", sep = "\t",
            row.names = FALSE)

# write.table(row.names(DEGs), col.names = F, quote = F, file = "DEGs.txt", sep = "\t",
#             row.names = FALSE)

# Get all expressed genes, take out their ID and create a txt (for ORA)

expressedGenesFullTable <- topTags(qlf, nrow(qlf$table))$table
expressedGenes <- row.names(expressedGenesFullTable)
write.table(expressedGenes,col.names = F, quote = F, file = "expressedGenes.txt", sep = "\t",
            row.names = FALSE)

# Combine fold-change and statistical value to create data (upload to GSEA)

GSEA_column <- (sign(expressedGenesFullTable$logFC)*-log10(expressedGenesFullTable$FDR))
GSEA <- cbind(expressedGenes,GSEA_column)
write.table(GSEA, col.names = F, quote = F, file = "GSEA.rnk", sep = "\t",
            row.names = FALSE)
...
points(plotData[upReg,], col="#ffa781", pch=20)
points(plotData[downReg,], col="#5b0e2d", pch=20)
text(plotData[upReg[1:3],], labels=upReg[1:3],col="#ffa781", pos=sample(c(1:2), size=10, replace=T), cex=0.8)
text(plotData[downReg[1:3],], labels=downReg[1:3],col="#5b0e2d", pos=sample(c(1:3), size=10, replace=T), cex=0.8)
...
points(plotData[upReg,], col="#ffa781", pch=20)
points(plotData[downReg,], col="#5b0e2d", pch=20)
text(plotData[upReg[1:3],], labels=upReg[1:3],col="#ffa781", pos=sample(c(1:3), size=10, replace=T), cex=0.8)
text(plotData[downReg[1:3],], labels=downReg[1:3],col="#5b0e2d", pos=sample(c(1:2), size=10, replace=T), cex=0.8)
